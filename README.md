...
- **Admin analytics** (`/admin`) — authenticated live visitor telemetry:
  page loads, searches, predictions, and tool usage per visitor IP.
...
| `ADMIN_USER`    | Admin portal username                              | `admin`                     |
| `ADMIN_PASSWORD`| Admin portal password                              | random per boot             |
| `DB_PATH`       | SQLite database path                               | `./colleges_v2.db`          |
| `FLASK_DEBUG`   | Set to `1` to enable the debug server (dev only)   | off                         |
...
- `/admin` and `/admin/analytics/data` require login (CSRF-protected,
  rate-limited against brute force; session cookies are `Secure`+`HttpOnly`).
- All SQL uses parameterized queries; telemetry input is sanitized and escaped.
- Public `/data` and `/api/*` endpoints are rate-limited per client IP.
- Run gunicorn as an unprivileged user; keep secrets in a 0600 env file
  (`/var/lib/college_project/college.env`) and the DB outside the web root.
